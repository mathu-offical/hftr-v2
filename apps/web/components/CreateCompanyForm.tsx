    ...
    >
      ...
      >
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-2 overflow-hidden p-3 sm:p-4">
                        ...
                        <li className="space-y-1 pt-1">
                          ...
                          <ul className="space-y-1">
                            {seedHierarchy.orphans.map((orphanSeed) => {
                              const item = engines.find((entry) => entry.key === orphanSeed.key);
                              if (!item) return null;
                              return (
                                <li key={item.key}>
                                  <EngineNavRow
          ...
          {error && (
            <p className="shrink-0 text-sm text-[var(--color-block)]">{error}</p>
          )}
        </div>
...
function EngineInspectorFrame(props: {
  ...
  const selectedSection = selectedTemplate
    ? engineCreateSection(selectedTemplate)
    : 'research';
