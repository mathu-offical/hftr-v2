...
| **G3** Paper trading loop | **Partial** | ValueRef finalizer on promote (D-057); fund approve→**settled** + module ledger (D-059); **pct + fixed** allocation resolve (D-061) **+ compile budget cap**; **trace→Values lineage deep links** (D-060); **D-087** liveGateBlocked / realizedLoss / regime_fit / balance cascade; **evidence mirror** into trend-linked shelves for day_trading promote; **position_exits** maintenance (breakeven/target/time_stop); synthetic regime **directionBias** + promote drain execution-spine-only (3/3 single-attempt fills verified); Alpaca bars→regime still optional; Playwright 3+7 | Remote CI e2e; credentialed Alpaca paper fill; fund_router auto-feed; enforceScopeStrict; RR/ATR scale-out ladder |
