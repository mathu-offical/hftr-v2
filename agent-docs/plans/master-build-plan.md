...
| **G3** Paper trading loop | **Partial** | ValueRef finalizer on promote (D-057); fund approve→**settled** + module ledger (D-059); **pct + fixed** allocation resolve (D-061) **+ compile budget cap**; **trace→Values lineage deep links** (D-060); **D-087** liveGateBlocked / realizedLoss / regime_fit / balance cascade; **evidence mirror** into trend-linked shelves for day_trading promote; **position_exits** (ATR/RR/**measurable_gain_take**/trail_stop/half-R BE/time_stop/session_close-if-opened-open) + **D-124** polarization×ATR-risk + **D-125** portfolio_heat compile gate + weighted valves + **D-126** POV child-slice plan + training_feedback + atr_stream resolver + fee ledger; catalog recovery phases on trees; synthetic regime **directionBias** + promote drain execution-spine-only; inline trade/scan drains scoped (DISPATCH/RESEARCH); **0040** repaired legacy `module_service_bindings_source_check`; **0043** training_feedback | Remote CI e2e; credentialed Alpaca paper fill; live atr_stream soak; POV partial-fill drain; control_snapshots persist every promote; fund_router auto-feed; enforceScopeStrict |
