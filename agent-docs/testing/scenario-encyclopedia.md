...
| ARCH-005 | Broken graph: trading node with no inbound data feed | Module stays `draft`; promote/active transition blocked; text-visible reason | Module status + chip copy; PATCH `module_graph_incomplete` + `reasons[]` | yes | D-024 draft gating; `activationGraphBlockers` |
