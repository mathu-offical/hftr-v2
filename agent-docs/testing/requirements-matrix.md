...
| REQ-CMP-002 | Company templates (blank, day_trading_starter, trend_research_lab) | agent-docs/product/product-spec.md §2 | **implemented** | packages/contracts/src/templates.ts; apps/web/components/CreateCompanyForm.tsx | companies.spec.ts template buttons | paper_sim | strategy_outcome | operational |
| REQ-CMP-003 | Company creation wizard UI | agent-docs/ui-ux/ui-spec.md §3 | **implemented** | apps/web/components/CreateCompanyForm.tsx; apps/web/app/companies/page.tsx | companies.spec.ts | platform | operator_transparency | operational |
| REQ-CMP-004 | Template setup Required/Set validation chips | agent-docs/product/product-spec.md §2 (D-024) | **implemented** | apps/web/components/canvas/ModuleSetupFields.tsx; apps/web/lib/module-setup.ts | companies.spec.ts Required chips; company-workspace.spec.ts inline save | paper_sim | operator_transparency, risk_controls | financial |
...
| REQ-ID | Title | Source | Status | Evidence | Scenarios | Venues | Axes | Safety |
|---|---|---|---|---|---|---|---|---|
| REQ-TST-001 | Contracts package vitest suite | packages/contracts/src/contracts.test.ts | **implemented** | contracts.test.ts 19 cases | pnpm test contracts | platform | operator_transparency | operational |
| REQ-TST-002 | Engine pipeline + dispatch + calc tests | packages/engine/src/ | **implemented** | pipeline.test.ts; dispatch.test.ts… | 34 engine test cases | paper_sim | risk_controls | safety_critical |
| REQ-TST-003 | paper-sim adapter tests | packages/adapters/src/paper-sim.test.ts | **implemented** | paper-sim.test.ts 4 cases | pnpm test adapters | paper_sim | execution_quality | financial |
...
| REQ-TST-005 | company-workspace.spec.ts Playwright | apps/web/e2e/company-workspace.spec.ts | **implemented** | company-workspace.spec.ts 153 lines | canvas/panels/assistant | paper_sim | operator_transparency | operational |
...
| REQ-TST-007 ⚠ | Full pipeline flow 3 Playwright | agent-docs/ui-ux/ui-spec.md §7 flow 3 | **deferred** | ui-spec Not yet covered | research→trade E2E | paper_sim | execution_quality, strategy_outcome | safety_critical |
...
- **REQ-TST-006**: doc-drift: flow 1 claimed in spec table but explicitly marked not Clerk-covered.
- **REQ-TST-007**: Synthetic spine exists in engine; no E2E driving full hop chain.
