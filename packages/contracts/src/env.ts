...
export const ENVIRONMENT_REQUIREMENTS: readonly EnvRequirement[] = [
  ...
  },
  {
    ...
    requiredIn: [],
    consumer: 'unused for auth (D-026 user keys only; reserved for offline tooling)',
  },
  {
  ...
  { name: 'CEREBRAS_API_KEY', requiredIn: [], consumer: 'unused for auth (D-026 user keys only)' },
  { name: 'FIREWORKS_API_KEY', requiredIn: [], consumer: 'unused for auth (D-026 user keys only)' },
  {
    name: 'OPENROUTER_API_KEY',
    requiredIn: [],
    consumer: 'unused for auth (D-026 user keys only)',
  },
