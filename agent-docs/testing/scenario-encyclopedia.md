...
| ISO-005 | User archives company A; B remains active | A returns 404 on scoped routes; B unaffected; A schedules disabled and skipped; execution context rejects A | Archive batch clears live/broker + pauses modules; `getOwnedCompany` / `resolveExecutionContext` exclude archived | yes | e2e `companies.spec.ts` archive 404 assertions; `archiveCompany` fixture |
...
| ARCH-005 | Broken graph: trading / research / trend / library / librarian missing required peers | Module stays `draft`; promote/active blocked; text-visible reason(s) | Module status + chip copy; PATCH `module_graph_incomplete` + `reasons[]`; 21 unit cases on `activationGraphBlockers` | yes | D-024 draft gating; `activationGraphBlockers` (trading←data_feed; research→library; trend←library/live_api; library←research/librarian; librarian←library/research) |
